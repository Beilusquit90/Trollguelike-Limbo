...
RuinesMap::RuinesMap()
{
	...
	inDoor = inDoor->genLimbo(3);
	inDoor->lleft = inDoor;
	////////////////////////////////
	std::vector<Limbo*>* vLimbo;

	auto zz = new Limbo(1);	vLimbo = zz->getvLimbo();		delete zz;

	std::cout << vLimbo->size() << "vlimbo size" << std::endl;
	int cc = 0;
	int z = 40;
	for (auto &x : *vLimbo)
	{
	...
	//myHero = TheSeed[0].inDoor;


	vBody = myHero->cell->body->getvBody();
...
//	}
	...
	cursy = 0;
	std::cout << "Ruines map gen end." << std::endl;
}


RuinesMap::RuinesMap(RuinesMap*rhs) {
	inDoor = rhs->inDoor;
	...
	outDoor = rhs->outDoor;
	myHero = rhs->myHero;
	std::cout <<"DANGER MOTHER FUCKERS!!!" << std::endl;
}
		...
		case 0:
			...
			case GLUT_LEFT_BUTTON:
				...
				if (1)if (state)
				{
					...
					//{
						//std::cout << "State in ->" << state << std::endl;
...
int RuinesMap::CreateSteps(int tx, int ty)
{
	...
	//std::cout <<"12-ty "<<12-ty<<"   tx "<<tx << std::endl;
	Limbo *objz = myHero->cell->getpatch(12 - ty, tx);
	if (12 - ty == 6 && tx == 6)
		return 0;
	#define RCT objz
	#define RC myHero->cell
	int	tempme = RC->me;
	Limbo * tempc = RC;
	if (RCT) {
		//std::cout << "Create steps" << std::endl;
		if (!RCT->body) {
			RCT->body = RC->body;
			RCT->me = RC->me;
			RC->me = 0;
...
void RuinesMap::Attack(Limbo*rhs, Limbo*rhl)
{
	rhl->body->MyKiller = rhs->body;
	rhl->body->hp -= (rhs->body->str);
	rhs->body->tiktak += (rhs->body->attackS);

}

int RuinesMap::Move(Body*rhsz,int flag)
{
#define RC rhs->cell 
#define RCR rhs->cell->lright 
#define RCL rhs->cell->lleft 
#define RCB rhs->cell->lbot 
#define RCT rhs->cell->ltop 
...
void RuinesMap::rMove(Limbo**x) // заставляет убогих, совершать рандомное движение. На крайний случай, anotherMove
{
...
void RuinesMap::WhoDie()
{
	...
	for (auto &x : *vBody)
	{
		...
		{
			if (!x->MyKiller)
				std::cout << "AGAIN THAT FKNG BUG!!! MIKILLER ALREADY DEAD!" << std::endl;
			Limbo *	temp = x->cell;
			if (x->inventory.size() > 0&& x->MyKiller) {
				(x->MyKiller->inventory).push_back(x->inventory.front());
				x->MyKiller = 0;
			}
...
{
    #define ifc int i = 0; levelSize[dx][dy] != 999 && levelSize[dx][dy] != 9 && i < bird.range; i++
	TactikMap[dx][dy] = 1;
	...
	for (auto &act : *vBody)
	{
		...
		if (act->tiktak <= 0)
			if (act->cell->body)
				...
				std::cout << " FFs... again bad cell. Role->" << act->cell->me << std::endl;
		
			//	std::cout << " MyHero Role->" << MyHero->body->role << std::endl;
...
void RuinesMap::DrawInterface()
{
	...
	/*	glVertex2f(cursy, cursx);
		...
		glVertex2f(cursy, cursx);
		glVertex2f(cursy, cursx - 20);
		glVertex2f(cursy, cursx);
		glVertex2f(cursy + 20, cursx - 20);

		glColor3f(1, 0, 0);
		for (int i = 2; i < 18;i++) {
			glVertex2f(cursy, cursx);
			glVertex2f(cursy + i, cursx - 20);
		}
		glColor3f(1, 1, 1);
		glEnd();

...
public:
	...
	void rMove(Limbo**x);					//временная функция, для рандомных шагов монстров.
	void flyDeath();					// функция отвечающая за смерть стрелл и заклинаний.
	int fly(MAmap&rhs);					//летающие стрелы и магия.
	int Move(Body*rhs, int flag);	// функция для ходьбы по 8 направлениям
	void Attack(Limbo*rhs, Limbo*rhl);//говорящее название
